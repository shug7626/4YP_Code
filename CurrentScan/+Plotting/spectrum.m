...
function [fig, s] = spectrum(s, p, set)
    ...
    s.cell1 = p.etac1 .* (ones(size(p.R1)) - p.R1) .* (ones(size(p.R1)) - exp(-p.a1 * p.thick1 * 1e-7)) .* (s.bs .* p.validE1);
    
    % Calculate for cell 2
    s.cell2 = p.etac2 .* (ones(size(p.R2)) - exp(-p.a2 * p.thick2 * 1e2)) .* (s.bs2 .* p.validE2);
